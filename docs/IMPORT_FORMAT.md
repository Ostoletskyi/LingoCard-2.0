...
### Размер шрифта в JSON
- Каноническое поле размера шрифта блока: `boxes[].style.fontSizePt`.
- Для обратной совместимости при импорте также поддерживается алиас `boxes[].fontPt` (если `style.fontSizePt` отсутствует).
- При экспорте пишется `style.fontSizePt`, а также дублируется `fontPt` для совместимости со старыми файлами.
