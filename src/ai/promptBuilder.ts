...
export type AiMode = "generate" | "repair" | "patch";
export type AiInputLanguage = "ALL" | "RU" | "DE" | "EN";
export type ChatRole = "system" | "user" | "assistant";
export type ChatMessage = { role: ChatRole; content: string };

/**
 * Patch envelope returned by AI in patch mode.
 * (We keep it explicit so the caller can validate with Zod on their side.)
 */
export type CardPatchEnvelope = {
  patch: Partial<Card>;
  errors?: string[];
};
...
const CARD_SCHEMA_GUIDE = `{
  ...
  "forms_prat": "string",
  "forms_p2": "string",
  "forms_aux": "haben|sein|\"\"",
  "forms_service": "string (exact format: er/sie/es <P3> - ich <Prat> - hat/ist <P2>)",

  "syn_1_de": "string", "syn_1_ru": "string",
  "syn_2_de": "string", "syn_2_ru": "string",
  "syn_3_de": "string", "syn_3_ru": "string",

  "ex_1_de": "string", "ex_1_ru": "string", "ex_1_tag": "praesens",
  "ex_2_de": "string", "ex_2_ru": "string", "ex_2_tag": "modal",
  "ex_3_de": "string", "ex_3_ru": "string", "ex_3_tag": "praeteritum",
  "ex_4_de": "string", "ex_4_ru": "string", "ex_4_tag": "perfekt",
...
const PATCH_SCHEMA_GUIDE = `{
  "patch": "Partial<Card> (only keys from Card schema allowed)",
  "errors": "optional string[]"
}`;

// --- System prompts (separated contracts, no ambiguity) ----------------------

const SYSTEM_GENERATE = [
  `You are LingoCard AI.`,
  `Output ONLY one valid JSON object of type Card.`,
  `No markdown. No comments. No backticks. No extra keys. No text around JSON.`,
  `If unknown use "" or [].`,
...
const SYSTEM_REPAIR = [
  ...
  `Output ONLY one valid JSON object of type Card.`,
  `No markdown. No comments. No backticks. No extra keys. No text around JSON.`,
  `Keep already-correct values and fix only what is necessary to match schema.`,
  `If unknown use "" or [].`,
  `All string fields must be single-line values (no embedded line breaks).`,
  `Schema:`,
  CARD_SCHEMA_GUIDE
...
const SYSTEM_PATCH = [
  `You are a JSON patch generator for an existing Card.`,
  `Output ONLY one valid JSON object matching the patch schema below.`,
  `No markdown. No comments. No backticks. No extra keys. No text around JSON.`,
  `Rules:`,
  `- Return {"patch": {...}, "errors"?: [...]} ONLY.`,
  `- "patch" may include ONLY keys that exist in Card schema.`,
  `- Do NOT overwrite correct existing values unless required to satisfy constraints.`,
  `- If a field is unknown, do not invent; prefer leaving it unchanged (omit from patch) unless schema requires it.`,
  `Patch schema:`,
  PATCH_SCHEMA_GUIDE,
...
const languageHint = (lang: AiInputLanguage): string => {
  switch (lang) {
    case "RU":
      return "Input language is Russian. Map to the best German infinitive first.";
    case "DE":
      return "Input language is German. Keep/normalize the German infinitive.";
    case "EN":
      return "Input language is English. Map to the best German infinitive first.";
    default:
